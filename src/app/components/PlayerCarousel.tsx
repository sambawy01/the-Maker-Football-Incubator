        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {featured.map((player) => (
                <Link key={player.id} to={`/players/${player.id}`} className="block">
                    <div className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 group h-[480px] relative">
                        ...
                        <div className="p-6 absolute bottom-0 left-0 w-full h-[45%] flex flex-col justify-end bg-gradient-to-t from-white via-white to-transparent">
                            ...
                            <div className="flex items-center gap-4 text-gray-500 text-sm mb-4">
                                <span className="flex items-center gap-1"><MapPin size={12} /> {player.gov}</span>
                                <span>•</span>
